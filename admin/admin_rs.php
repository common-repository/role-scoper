...
// Migration Advisor
require_once( dirname(__FILE__).'/migration/admin-load_rsu.php' );
...
class ScoperAdmin
{
	...
	function __construct() {
		...
		add_action('admin_menu', [$this, 'actRedirectOldURL'], 999);
		...
		|| (!empty($_REQUEST["page"]) && in_array($_REQUEST["page"], array('rs-options', 'rs-site_options', 'rs-default_options'))) 
		|| (!empty($plugin_page_cr) && ('rs-migration-preview' != $_REQUEST["page"]))
		) {
	...
	function actRedirectOldURL() {
		foreach ([
			'rs-migration-advisor' => 'rs-migration-preview',
	...
	function dashboard_notice() {
		if (! is_super_admin() || defined('PRESSPERMIT_VERSION')) {
			return;
		}

		require_once( dirname(__FILE__).'/misc/version_notice_rs.php' );
		$message = scoper_pp_msg();
        $message_css = 'background-image: url(' . plugins_url( '', SCOPER_FILE ) . '/admin/migration/images/pp-message.png);';
		?>
		<div class='updated rs-upgrade rs-upgrade-box' style='<?php echo $message_css ?>' id='rs_dashboard_message'><?php echo $message ?>
		</div>
		<?php
	}
