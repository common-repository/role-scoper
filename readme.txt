=== Plugin Name ===
Contributors: publishpress, kevinB, stevejburge
...
<strong style="color:#c00">Role Scoper is no longer actively developed.</strong> For compatibility with current WordPress and plugin versions, please migrate to the successor plugin, <a href='https://wordpress.org/extend/plugins/press-permit-core/'>PublishPress Permissions</a> or <a href='https://publishpress.com/presspermit'>PublishPress Permissions Pro</a>. An import script is provided.
...
= Is Role Scoper still supported? =

We do not plan any further development of the Role Scoper code base. Our support team is glad to advise you about migrating to PublishPress Permissions. 

= How does migration to PublishPress Permissions work? =

The new permissions model provides better flexibility and performance, but is different enough that an import process is needed. Role Scoper now provides a preview of how PublishPress Permissions will import your configuration. For screen captures of the import preview and execution, see our <a href="https://publishpress.com/knowledge-base/migrate-role-scoper/">Migration How To Guide</a>.

While most Role Scoper configurations can be automatically imported, some may require manual followup.  If your permissions do not map over as expected, <a href="https://publishpress.com/contact/">contact our support team</a> for assistance.

= Are all my Role Scoper customizations covered by PublishPress Permissions (Free), or do I need Permissions Pro? =

The most commonly used permissions customizations are now handled by the free edition of PublishPress Permissions.  This includes blocking or enabling reading or editing access for specific posts, pages, categories or terms.  

Pro features include Direct file access restriction, teaser, bbPress forum access control, BuddyPress permission groups, and access control for custom post statuses. For details, see the <a href="https://publishpress.com/wp-content/uploads/2020/03/RoleScoper-Permissions-Feature-Grid.pdf">Role Scoper / Permissions Comparison Grid</a>. 

= What does Role Scoper to my roles? =
...
* XML-RPC support
* Integrates with the [Revisionary plugin](https://wordpress.org/extend/plugins/revisionary/) for moderated revisioning of published content.
* Supports custom Post Types and Taxonomies (when defined using WP schema by a plugin such as [Custom Post Type UI](https://wordpress.org/extend/plugins/custom-post-type-ui/) 
* Extensive WP-mu support

= How does Role Scoper compare to a role editor like PublishPress Capabilities? =

Role Scoper's functionality is entirely different and complementary to a conventional role editor.  Role editors typically do little more than alter WordPress' definition of the capabilities included in each role.  That's a valuable task, and in many cases will be all the role customization you need.  Since role editor modifications are usually stored in the main WordPress database, they remain even if the plugin is deactivated.

Role Scoper is useful when you want to customize access to specific content, not just blog-wide.  It will work with the WP roles as a starting point, whether customized by a role editor or not.  To see how Role Scoper's role definitions correlate to your WordPress roles, navigate to Roles > Options > RS Role Definitions in your blog admin.  Role Scoper's modifications remain only while it stays active.
...
A slightly outdated Usage Guide is included in the plugin folder.  It includes both an overview of the permissions model and a How-To section with step by step directions.
...
**The Events Calendar** : Not compatible as of TEV 1.6.3.
...
= 1.5 =
Please upgrade for important compatibility and migration information.

== Changelog ==

= 1.5 - 5 Mar 2020 =
* Change : Plugin obsolete notice, offer to migrate to PublishPress Permissions
* Feature : PublishPress Permissions Migration Preview 
* Fixed : Non-administrators had fatal error in wp-admin (Too few arguments to function wp_maybe_grant_site_health_caps)
